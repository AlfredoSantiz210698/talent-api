...
module.exports = {
  ...
  */
  origin: function (currentOrigin) {
    const currentNodeEnv = Env.get("NODE_ENV", "dev");
    const nodeEnvProd = ["prod", "production"];
    const nodeEnvDev = ["dev", "development"];
    let origins = [];

    if (nodeEnvDev.includes(currentNodeEnv)) {
      origins = Env.get("ORIGINS_ALLOWED_DEV", "").split("|");
    }

    if (nodeEnvProd.includes(currentNodeEnv)) {
      origins = Env.get("ORIGINS_ALLOWED_PROD", "").split("|");
    }
