...
const { getAuthJwtPayload, getRequestFilter, promiseResolver } =
  use("App/Utils");
...
class TalentController {
  ...
  async store({ request, response, auth }) {
    try {
      const { user_role_id } = getAuthJwtPayload(auth);
      ...
      payload.user_role_id = user_role_id;

      const [talent, talentError] = await promiseResolver(
        await Talent.create(payload)
      );

      if (talentError) {
        throw new RecordNotCreatedException("Talent couldn't be created");
      }

      return response.status(201).json(talent);
    } catch (error) {
      return response.status(400).json(exceptionResponseFactory(error));
    }
  }
